...
public class DrawerExample extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        menuList.getItems().addAll("功能1：读入文本", "功能2：展示有向图", "功能3：查询桥接词", "功能4：生成新文本",
                 "功能5：最短路径", "功能6：随机游走");
    ...
    //第五问使用
    MultiShortestPath multiShortestPath = new MultiShortestPath();
    public static int shortestPathNumbers = 0;
    public static int shortestPathLengths = 0;

    public int[][] calcShortestPath(String word1, String word2){
        multiShortestPath.setShortestPathNumbers(0);
        int[][] adjMatrix = myGraph.getAdjMatrix();
        int start5 = myGraph.getIndex(myGraph.getWords(), word1);
        int end5 = myGraph.getIndex(myGraph.getWords(), word2);
        System.out.println("start5 和end5是:" + start5 + " " + end5);
        //String str5 = shortestPath(adjMatrix, start5, end5).toString();
        ArrayList<ArrayList<Integer>> ret5_2 = multiShortestPath.findMultiPath(adjMatrix, start5, end5);
        System.out.println("空缺检查" + ret5_2);
        if(ret5_2==null){

        }
        for (ArrayList<Integer> item0 : ret5_2){
            System.out.println("返回后:" + item0 + " " + item0.size());
            shortestPathLengths = item0.size();
        }
        shortestPathNumbers = multiShortestPath.getShortestPathNumbers();
        int[][] ret5_3 = new int[shortestPathNumbers][shortestPathLengths];
        for (int item0 = 0; item0<shortestPathNumbers;item0++){
            for(int item1 = 0;item1<shortestPathLengths;item1++){
                ret5_3[item0][item1] = ret5_2.get(item0).get(item1);
                System.out.println("中间项:" + item0 + " " + item1 + " " + shortestPathNumbers +  " " + shortestPathLengths + " " + ret5_3[item0][item1]);
            }
        }
        //System.out.println(str5);
        //return "hello";
    ...
    public void setPages(int i){
        switch (i){
            ...
            case 4:
                ...
                TextField textField5_3 = new TextField("任意起点");
                // 设置输入框的最小宽度和高度
                ...
                button5_3.prefWidthProperty().bind(pages[4].widthProperty().multiply(0.3));
                button5_3.prefHeightProperty().bind(pages[4].heightProperty().multiply(0.2));

                // 创建垂直布局
                VBox vbox5_1 = new VBox(10); // 设置垂直间距为10
                vbox5_1.getChildren().addAll(textField5_1, textField5_2,textField5_3, button5_1, button5_2, button5_3);
                ...
                button5_1.setOnAction(event -> {
                    ...
                    //选做的第五问
                    int[][] text5_int = calcShortestPath(text5_1, text5_2);
                    String[][] text5_result = new String[shortestPathNumbers][shortestPathLengths];

                    for(int item0=0;item0<shortestPathNumbers;item0++){
                        for (int item1 = 0;item1<shortestPathLengths;item1++){
                            text5_result[item0][item1] = myGraph.getWords()[text5_int[item0][item1]];
                            System.out.println("转化后字符串:" + text5_result[item0][item1]);
                        }
                    }
                    ...
                    // 输出带有行号和箭头的数组
                    for (int item = 0; item < shortestPathNumbers; item++) {
                        output.append("第").append(item + 1).append("行: ");
                        for (int j = 0; j < shortestPathLengths; j++) {
                            output.append(text5_result[item][j]);
                            if (j < shortestPathLengths-1) {
                                output.append(" -> ");
                            }
                        }
                        output.append("\n");
                    ...
                    if(output.isEmpty()){
                        textField5_0 = new TextField("no path");
                    } else {
                        textField5_0 = new TextField(output.toString());
                    }
                    // 将标签添加到根节点中
                    pages[4].getChildren().add(textField5_0);
                });
                ...
                button5_2.setOnAction(event->{
                    ...
                    System.out.println("text是" + text5_1 + " " + text5_2);

                    //选做的第五问
                    int[][] text5_int1 = calcShortestPath(text5_1, text5_2);
                    System.out.println("--------------------------");
                    String[][] text5_result = new String[shortestPathNumbers][shortestPathLengths];

                    for(int item0=0;item0<shortestPathNumbers;item0++){
                        for (int item1 = 0;item1<shortestPathLengths;item1++){
                            text5_result[item0][item1] = myGraph.getWords()[text5_int1[item0][item1]];
                            System.out.println("转化后字符串:" + text5_result[item0][item1]);
                        }
                    }
                    for(int item = 0; item<shortestPathNumbers;item++){
                        //对每一张图片创建最短路径示意图
                        StringBuilder result = new StringBuilder();
                        for (String word : text5_result[item]) {
                            result.append(word).append("->");
                        }

                        // 删除最后一个箭头后的空格
                        result.delete(result.length() - 2, result.length());
                        myGraph.start_marked(new Stage(), result.toString(), "MultiPicture" + (item + 1) + ".png");

                        Pane markedPane = myGraph.createAnotherGraphPane(result.toString());
                ...
                button5_3.setOnAction(event->{
                    String text5_3 = textField5_3.getText();
                    //System.out.println("起始点是:" + text5_3 + " " +  myGraph.getIndex(myGraph.getWords(), text5_3));
                    AllPaths allPaths = new AllPaths(myGraph.getAdjMatrix(), myGraph.getIndex(myGraph.getWords(), text5_3), myGraph);
                    TextField textField5_4 = new TextField(allPaths.dijkstra(myGraph.getAdjMatrix()));
                    vbox5_1.getChildren().add(textField5_4);
                });
                break;
